...
class SortSourceService:
    ...
    def sort_service(self, query: str, search_results: List[dict]) -> List[dict]:
        relevant_docs = []
        query_embedding = self.embedding_model.encode(query)

        for res in search_results:
            content = res.get("content")  # Safe access
            if not isinstance(content, str) or not content.strip():
                continue  # Skip if content is None, empty, or not a string
